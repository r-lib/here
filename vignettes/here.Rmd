...
## Enforce project-oriented workflow

Even if you always work inside a project, your collaborators may not be aware.
`here()` falls back to the current working directory when no project can be found.
This means that your code does not find the correct files even if you use `here()` everywhere.

To give an early warning when this situation arises, use the following pattern in your scripts and reports:
