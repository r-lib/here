...
[^legacy]: Prior to version 1.0.0, it was recommended to load[^attached] the here package via `library(here)`.
This still works, but is no longer the recommended approach.

- The location of the current script or notebook within the project is declared
- The project root is initialized, consistent with the location of the current script or notebook
...
This would have failed if the working directory had been e.g. the user's home or an unrelated project.
...
To eliminate potential confusion, `here::i_am()` accepts a `contents` argument.

FIXME: `uuid` argument`

If specified, it should be a regular expression.
The first 100 lines of the script are matched against this regular expression.

It is a good idea to pass in a universally unique identifier (UUID) as this argument.
...
There may be cases when it is desirable to reset the project root.
