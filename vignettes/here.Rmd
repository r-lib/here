...
The declaration of the active file via `here::i_am()` also protects against accidentally running the script
from a working directory outside of your project.
...
In the future, a helper function will assist with installing and updating suitably formatted `here::i_am()` calls
in your scripts and reports.
...
The conflicted package offers an alternative:
it detects that `here()` is exported from more than one package and allows you to use neither until you indicate a preference.
...
Even if a file location is reused across projects (e.g. two projects contain a "prepare/data.R" file),
the files can be identified correctly if the `uuid` argument in the `here::i_am()` call is different.
...
It is advisable to [start a fresh R session](https://rstats.wtf/source-and-blank-slates) as often as possible,
especially before focusing on another project.
